...
class downloads(Screen):
	def __init__(self, session):
		...
		self['actions'] = ActionMap(['OkCancelActions', 'ColorActions'], {'cancel': self.close, 'ok':self.save,'green':self.save}, -2)
						...
						if config.plugins.xtraEvent.extra2.value == True:
							...
							if not os.path.exists(dwnldFile):
								try:
									...
									open(dwnldFile, 'wb').write(requests.get(url, stream=True, allow_redirects=True).content)
									downloaded += 1
									extra2_downloaded = extra2_downloaded
									self.prgrs(downloaded, n)
									self['info'].setText(_("{}, backdrop downloaded from EXTRA...".format(title.upper())))
									self.brokenImageRemove()
								except:
...
# infos #################################################################
						...
						if not os.path.exists(info_files):
							try:
								...
								if imdb_id:
									...
									w = open(info_files,"wb").write(json.dumps(info_omdb))
									info_downloaded += 1
									infodownloaded = downloaded
									self.prgrs(downloaded, n)
									self['info'].setText(_("{}, downloaded from INFO...".format(title.upper())))
				...
				self['info2'].setText("{}\nposter; (tmdb :{}, tvdb :{}, maze :{}, fanart :{})\nbackdrop; ((tmdb :{}, tvdb :{}, fanart :{}))\nbanner :{}\ninfos :{}".format(dt, str(tmdb_poster_downloaded), str(tvdb_poster_downloaded), str(maze_poster_downloaded), str(fanart_poster_downloaded), str(tmdb_backdrop_downloaded), str(tvdb_backdrop_downloaded), str(fanart_backdrop_downloaded), str(banner_downloaded), str(info_downloaded)))
				with open("/tmp/up_report", "a+") as f:
					f.write("end : {}\nposter; (tmdb :{}, tvdb :{}, maze :{}, fanart :{})\nbackdrop; ((tmdb :{}, tvdb :{}, fanart :{}))\nbanner :{}\ninfos :{}".format(dt, str(tmdb_poster_downloaded), str(tvdb_poster_downloaded), str(maze_poster_downloaded), str(fanart_poster_downloaded), str(tmdb_backdrop_downloaded), str(tvdb_backdrop_downloaded), str(fanart_backdrop_downloaded), str(banner_downloaded), str(info_downloaded)))
			else:
				Tools.Notifications.AddNotification(MessageBox, _("NO INTERNET CONNECTION !.."), MessageBox.TYPE_INFO, timeout = 10)
